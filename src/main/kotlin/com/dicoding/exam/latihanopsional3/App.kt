...
fun manipulateString(str: String, int: Int): String {
    val numString = str.filter { it.isDigit() }
    val num = if (numString.isNotEmpty()) numString.toInt() else int
    return if (numString.isNotEmpty()) {
        val result = num * int
        str.replace(numString, result.toString())
    } else {
        "$str$int"
    }
}
